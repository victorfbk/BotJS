...
function escreverArquivo(nomeArquivo, conteudo) {
    fs.writeFile(nomeArquivo, conteudo, (err) => {
        if (err) {
            console.error(`Erro ao escrever o arquivo ${nomeArquivo}:`, err);
        } else {
            console.log(`O conteúdo foi salvo no arquivo ${nomeArquivo}.`);
        }
    });
}
...
function mostrarFilmes(tabela, indiceInicial, primeiraExecucao) {
    ...
    if (primeiraExecucao) {
        ...
        rl.question('Escolha o número do filme para iniciar: ', (resposta) => {
            ...
            escreverArquivo('Filmeatual.txt', filmeEscolhido[1]);
            const proximoIndice = (indiceInicialEscolhido + 1) % tabela.length;
            const proximoFilme = tabela[proximoIndice][1];
            escreverArquivo('Proxfilme.txt', proximoFilme);
